...
import { X, ExternalLink, Github, Code, Database, Palette } from 'lucide-react';
        ...
        <div className="relative">
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-center gap-3">
              <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium border ${getCategoryIcon(project.category)} bg-muted/50 text-muted-foreground border-border`}>
            ...
            <div className="absolute inset-0 bg-gray-900/80 rounded-lg flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-300">
              <div className="flex gap-4">
                <a
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-foreground/10 text-foreground rounded-lg hover:bg-foreground/20 transition-all duration-300 border border-foreground/20"
                >
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {project.technologies.map((tech) => (
                <span 
                  key={tech}
                  className={`px-3 py-2 text-sm rounded-lg border ${getTechColor(tech)}`}
                >
                  {tech}
                </span>
              ))}
          ...
          <div className="mb-6">
            ...
            <ul className="space-y-2 text-muted-foreground">
              ...
              <li className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 bg-primary rounded-full mt-2 flex-shrink-0"></span>
                <span>Cross-browser compatibility and accessibility features</span>
              </li>
            </ul>
          </div>
