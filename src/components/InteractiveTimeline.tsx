...
import { Calendar, MapPin, Building2, GraduationCap, Briefcase } from 'lucide-react';
...
const timelineData: TimelineItem[] = [
  {
    ...
    technologies: ["Laravel", "React.js", "Vue.js", "MySQL", "TailwindCSS"],
  ...
  },
  {
    ...
    technologies: ["Java", "Python", "JavaScript", "Database Design", "Software Engineering"],
    location: "Jakarta, Indonesia"
  }
];
        ...
        {timelineData.map((item, index) => (
          <div
            ...
            className={`relative pl-20 transition-all duration-300 cursor-pointer ${
              selectedItem === item.id ? 'transform scale-[1.02]' : 'hover:transform hover:scale-[1.01]'
            }`}
            onClick={() => setSelectedItem(selectedItem === item.id ? null : item.id)}
          >
            {/* Timeline Dot */}
            <div className={`absolute left-6 w-4 h-4 rounded-full border-2 bg-background transition-all duration-300 ${
            ...
            <div className={`portfolio-card transition-all duration-300 ${
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                <div className="flex items-center gap-3">
                  <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium border ${getTypeColor(item.type)}`}>
